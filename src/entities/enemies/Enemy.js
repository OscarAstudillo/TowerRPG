...
export default class Enemy extends Phaser.GameObjects.Container {
    constructor(scene, path, levelDifficulty, type = 'normal') {
        ...
        if (type === 'normal') { 
            ...
            if (biome === 'forest') color = 0x008000;
            if (biome === 'mountain') color = 0x8b4513;
            if (biome === 'volcano') color = 0xff4500;
        }
        else if (type === 'tank') { 
            hpBase = 250; speedBase = 0.6; armor = 5; size = 26;
            if (biome === 'forest') color = 0x556b2f;
            if (biome === 'mountain') { color = 0x708090; armor += 5; }
            if (biome === 'volcano') { color = 0x800000; hpBase = 300; }
        }
        else if (type === 'speed') { 
        ...
        if (this.bodyShape) {
            ...
            if (biome === 'volcano') this.bodyShape.setStrokeStyle(2, 0xffff00);
    ...
    die(killedByPlayer) { if (!this.scene) return; const scene = this.scene; if (killedByPlayer) { if (scene.onEnemyKilled) scene.onEnemyKilled(this); } else { if (scene.onEnemyLeaks) scene.onEnemyLeaks(this.leakDamage); } this.destroy(); if (scene.checkWaveStatus) { scene.time.delayedCall(100, () => { if (scene && scene.checkWaveStatus) scene.checkWaveStatus(); }); } }
}