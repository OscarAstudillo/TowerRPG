...
export default class Enemy extends Phaser.GameObjects.Container {
    constructor(scene, path, levelDifficulty, type = 'normal') {
        ...
        if (type === 'normal') { 
            ...
            if (levelId === 2) { color = 0x8B4513; armor = 3; } 
            else if (levelId === 3) { color = 0x2F4F4F; armor = 10; hpBase = 300; } 
        }
        else if (type === 'speed') { 
            hpBase = 60; speedBase = 1.4; color = 0xffff00; size = 16; goldDrop = 15; 
            if (levelId === 2) { speedBase = 1.6; color = 0xFFD700; } 
            else if (levelId === 3) { speedBase = 1.5; color = 0xFF4500; } 
        }
        else if (type === 'healer') { 
        ...
        this.originalColor = color; // Guardar color original para tintes
        ...
        if (levelId > 1 && !type.startsWith('boss')) this.bodyShape.setStrokeStyle(1, 0x000000);
    ...
    // --- NUEVO: LÓGICA DE ESTADOS ---
    updateDebuffs(delta) {
        // 1. Quemadura (DoT Rojo)
        if (this.debuffs.burn.active) {
            this.debuffs.burn.timer -= delta;
            this.debuffs.burn.tickTimer += delta;
            if (this.debuffs.burn.tickTimer >= 500) { // Tick cada 0.5s
                this.takeTrueDamage(this.debuffs.burn.damage, '#ff4500'); // Naranja fuego
                this.debuffs.burn.tickTimer = 0;
            }
            if (this.debuffs.burn.timer <= 0) {
                this.debuffs.burn.active = false;
                this.clearTint();
            }
        }
        ...
        if (this.debuffs.poison.active) {
            ...
            if (this.debuffs.poison.tickTimer >= 1000) { // Tick cada 1s
                this.takeTrueDamage(this.debuffs.poison.damage, '#00ff00'); // Verde
                this.debuffs.poison.tickTimer = 0;
            }
            if (this.debuffs.poison.timer <= 0) {
                this.debuffs.poison.active = false;
                this.clearTint();
            }
        }

        // 3. Congelación (Slow Azul)
        if (this.debuffs.freeze.active) {
            this.debuffs.freeze.timer -= delta;
            if (this.debuffs.freeze.timer <= 0) {
                this.debuffs.freeze.active = false;
                this.clearTint();
            }
        }

        // 4. Stun (Parálisis Amarilla)
        if (this.debuffs.stun.active) {
            this.debuffs.stun.timer -= delta;
            if (this.debuffs.stun.timer <= 0) {
                this.debuffs.stun.active = false;
                this.clearTint();
            }
        }
    }

    // Método para aplicar un estado desde fuera (Proyectiles/Torres)
    applyStatusEffect(effect) {
        if (!effect || !this.active) return;
        if (this.isShielded) return; // Inmune si tiene escudo

        // type: 'burn', 'poison', 'freeze', 'stun'
        // val: daño o porcentaje
        // duration: ms

        if (effect.type === 'burn') {
            this.debuffs.burn.active = true;
            this.debuffs.burn.damage = Math.max(this.debuffs.burn.damage, effect.val); // Queda el daño más alto
            this.debuffs.burn.timer = effect.duration;
            this.bodyShape.setFillStyle(0xff4500); // Teñir visualmente
        }
    ...
    // Daño puro (ignora armadura) para DoTs
    ...
    clearTint() {
        // Restaurar color original si no hay otros debuffs prioritarios
        if (this.active && this.bodyShape) {
            this.bodyShape.setFillStyle(this.originalColor);
        }
    }

    // ... (Mantener useBossSkill, performHeal, takeDamage normal, checkAttackPlayer, die) ...
    // COPIAR EL RESTO DE MÉTODOS EXISTENTES AQUÍ
