...
export default class MainMenuScene extends Phaser.Scene {
    constructor() {
        ...
        // ESTILOS GLOBALES DE TEXTO (Para legibilidad)
    ...
    switchTab(tabKey) {
        ...
        // Mostrar seleccionado y refrescar
    ...
    // --- ALERTA CENTRALIZADA (MEJORA VISUAL) ---
    ...
    // --- VISTA HÉROE MEJORADA ---
    createHeroView(w, h, cx, cy) {
        this.heroLevelText = this.add.text(cx, h * 0.18, '', { ...this.fontHeader, fontSize: '24px', color: '#00ffff' }).setOrigin(0.5);
        ...
        // Panel Stats
        const statsBg = this.add.rectangle(leftX, cy + 20, 350, 500, 0x000000, 0.6).setStrokeStyle(2, 0x555555);
        this.heroContainer.add(statsBg);
        this.heroStatsText = this.add.text(leftX - 160, contentY - 80, '', { ...this.fontBody, fontSize: '16px', lineHeight: 24 });
        this.heroContainer.add(this.heroStatsText);
        
        // Contenedor Equipamiento (Interactivo)
    ...
    refreshHero() {
        ...
        this.heroStatsText.setText(`CLASE: [ ${clsName} ]\n\n-- ATRIBUTOS BASE --\n❤️ Vida: ${Math.floor(s.hp)}/${s.maxHp}  ⚔️ Daño: ${s.damage}\n🛡️ Defensa: ${s.defense}  ⚡ Delay: ${s.attackSpeed}ms\n\n-- EXTRAS --${details}`);
        this.heroLevelText.setText(`NIVEL ${hero.level} (XP: ${hero.xp}/${hero.maxXp})`);
        this.pointsText.setText(`PUNTOS DE STAT: ${hero.statPoints}`);

        // --- RENDERIZADO DE SLOTS INTERACTIVOS ---
        this.equippedTextContainer.removeAll(true);
        const startX = this.heroStatsText.x;
        let startY = this.heroStatsText.y + 320; // Bajamos un poco más
        
        this.equippedTextContainer.add(this.add.text(startX, startY, "-- EQUIPAMIENTO (Clic para gestionar) --", { ...this.fontBody, color: '#aaa', fontStyle: 'italic'}));
        startY += 30;

        const slots = [
            { key: 'mainHand', label: '🗡️ Arma', cat: 'weapon' },
            { key: 'offHand', label: '🛡️ Off', cat: 'armor' }, // Offhand suele estar en armor
            { key: 'armor', label: '👕 Armadura', cat: 'armor' },
        ...
        slots.forEach(slot => {
            ...
            // Fondo del Slot
            const slotBg = this.add.rectangle(startX + 150, startY + 10, 320, 30, 0x222222).setOrigin(0.5).setInteractive({useHandCursor: true});
            ...
            const valTxt = this.add.text(startX + 100, startY, name, { ...this.fontBody, color: color, fontStyle: 'bold' });
            
            // LÓGICA DE CLIC EN SLOT
            slotBg.on('pointerdown', () => {
                if (!item) {
                    // IR A MOCHILA FILTRADA
                    this.inventoryCategory = slot.cat;
                    this.switchTab('inventory');
                } else {
                    // MOSTRAR DETALLES Y DESEQUIPAR
    ...
    // --- VISTA TORRES (CON REDIRECCIÓN) ---
    createTowersView(w, h, cx, cy) {
        // ... (misma estructura, solo mejoramos los slots vacíos)
        ...
        types.forEach((type, i) => {
            ...
            const title = this.add.text(x, y, names[i], this.fontHeader).setOrigin(0.5); 
            this.towersContainer.add(title);
            const statsText = this.add.text(x, y + 100, "Stats...", { ...this.fontBody, color: '#aaa', align: 'center' }).setOrigin(0.5); 
            ...
            for (let s = 1; s <= 2; s++) {
                ...
                slotBg.on('pointerdown', () => { 
                    ...
                    if (item) { 
                        ...
                        // IR A MOCHILA
                        this.inventoryCategory = 'tower_part';
                        this.switchTab('inventory');
                    }
                });
    ...
    // ... refreshTowersView (IGUAL, solo asegura que usa los estilos nuevos si creaste textos nuevos, pero aquí solo actualizamos texto existente) ...
    refreshTowersView() {
        const types = ['archer', 'cannon', 'mage'];
        types.forEach(type => {
            ...
            for (let s = 1; s <= 2; s++) {
                ...
                if (txtObj) { 
                    if (item) { 
                        ...
                        txtObj.setText("Slot Vacío (Clic para equipar)"); 
                        txtObj.setColor('#aaaaaa'); 
                    } 
                }
            }
        });
    ...
    // refreshTalents y learnTalent se mantienen IGUALES en lógica, solo asegúrate de que usen estilos legibles si cambias algo. 
    // Por brevedad, asumo que usas el código del paso anterior que ya funcionaba bien, 
    // pero recuerda añadir stroke a los textos si se ven mal.
    refreshTalents() {
        const hero = getCurrentHero();
        ...
        tiers.forEach(tierLevel => {
            ...
            tierTalents.forEach((talent, idx) => {
                ...
                let color = 0x333333; if (isSelected) color = 0x006400; else if (isBlocked || !isUnlocked) color = 0x111111;
    ...
    learnTalent(talent) {
        if (RPGSystem.spendTalentPoint(talent.id, 1)) { 
            updatePlayerStats(); SaveSystem.save(); this.refreshTalents();
            this.showCentralAlert(`¡TALENTO APRENDIDO: ${talent.name}!`, '#00ff00');
        }
    }

    // --- INVENTARIO (CON AVISO MEJORADO) ---
    createInventoryView(w, h, cx, cy) {
        ...
        const bg = this.add.rectangle(0, 150, 300, 450, 0x000000, 0.9).setStrokeStyle(2, 0xffffff);
        this.detailTitle = this.add.text(0, -50, "", { ...this.fontHeader, fontSize:'18px', align: 'center', wordWrap: {width: 280} }).setOrigin(0.5);
        this.detailStats = this.add.text(0, 50, "", { ...this.fontBody, fontSize: '13px', align: 'left', wordWrap: {width: 280} }).setOrigin(0.5);
        
        this.equipBtn = this.createActionButton(0, 200, "EQUIPAR", () => this.actionEquip(), 0x006400);
        this.fuseBtn = this.createActionButton(0, 250, "FUSIONAR...", () => this.initiateFusion(), 0x00008b);
        this.sellBtn = this.createActionButton(0, 300, "VENDER", () => this.actionSell(), 0x8b0000);
        
        this.itemDetailContainer.add([bg, this.detailTitle, this.detailStats, this.equipBtn, this.fuseBtn, this.sellBtn]);
        
        // Modales de fusión se mantienen igual, solo usa showCentralAlert en executeFusion
        this.createFusionModals(cx, cy);
    }

    createFusionModals(cx, cy) {
        // ... (Copia el código de creación de fusionListModal y fusionConfirmModal del paso anterior)
        // Solo asegúrate de que usen this.fontTitle, this.fontBody, etc. para que se vean bien.
        ...
        const lLabel = this.add.text(0, -180, "CANDIDATO A", { ...this.fontHeader, color: '#00ffff' }).setOrigin(0.5);
        this.fusionItem1Info = this.add.text(0, 0, "", { ...this.fontBody, align: 'center' }).setOrigin(0.5);
        ...
        const rLabel = this.add.text(0, -180, "CANDIDATO B", { ...this.fontHeader, color: '#ff00ff' }).setOrigin(0.5);
        this.fusionItem2Info = this.add.text(0, 0, "", { ...this.fontBody, align: 'center' }).setOrigin(0.5);
        ...
        const confirmTxt = this.add.text(0, 250, "¡PROBAR SUERTE!", this.fontBtn).setOrigin(0.5);
        confirmBtn.on('pointerdown', () => this.executeFusion());
        const cancelConfirm = this.add.text(0, 320, "Volver", { ...this.fontBtn, color: '#aaa' }).setInteractive({useHandCursor:true}).setOrigin(0.5);
    ...
    createInvCategoryBtn(x, y, label, cat) {
        ...
        btn.on('pointerdown', () => { 
            ...
            this.itemDetailContainer.setVisible(false); 
            this.refreshInventory(); 
            // Update visual state (simple redraw of tabs)
            this.createInventoryView(this.scale.width, this.scale.height, this.scale.width/2, this.scale.height/2);
        });
        this.invContainer.add(btn);
    }

    // ... refreshInventory, selectItem, etc. SE MANTIENEN CON LA LÓGICA DE COMPARACIÓN ...
    // Solo asegúrate de usar this.fontBody en los textos generados.
    ...
    refreshInventory() {
        ...
        const filteredItems = gameState.inventory.filter(i => { if (!i) return false; if (this.inventoryCategory === 'mats') return false; if (this.inventoryCategory === 'all') return i.type !== 'tower_part'; if (this.inventoryCategory === 'tower_part') return i.type === 'tower_part'; 
            // Filtro específico para slots (arma, armadura...)
    ...
    createForgeView(w, h, cx, cy) {
        ...
        this.recipeTitle = this.add.text(0, -60, "Selecciona Receta", { ...this.fontHeader, align: 'center', wordWrap: {width: 260} }).setOrigin(0.5);
        this.recipeInfo = this.add.text(0, 50, "", { ...this.fontBody, align: 'center', wordWrap: {width: 260} }).setOrigin(0.5);
    ...
    // ... helpers createForgeCatBtn, refreshForge, selectNormalRecipe, selectTowerPart ...
    ...
    executeCraftItem(recipe, rarityKey) {
        ...
        if (result.success) { 
            gameState.gold -= cost; gameState.inventory.push(result.item); this.goldText.setText(`ORO: ${gameState.gold}`); SaveSystem.save(); 
            // AVISO CENTRAL
            const color = '#' + RARITY[rarityKey].color.toString(16).padStart(6,'0');
            this.showCentralAlert(`¡FORJA EXITOSA!\n${result.item.name}`, color);
    ...
    executeCraftTower(type, rarityKey) {
        ...
        if (result.success) { 
            gameState.inventory.push(result.item); this.goldText.setText(`ORO: ${gameState.gold}`); SaveSystem.save(); 
            // AVISO CENTRAL
            const color = '#' + RARITY[rarityKey].color.toString(16).padStart(6,'0');
            this.showCentralAlert(`¡INGENIERÍA EXITOSA!\n${result.item.name}`, color);
        } else { this.forgeMsg.setText(`ERROR: ${result.error}`); this.forgeMsg.setColor('#ff0000'); }
    }
    ...
    // ... helpers: swapping, forceUnequip, removeItemFromInventory, actionEquip, actionSell ...
    // Estos se mantienen igual que en la versión anterior, asegúrate de tenerlos.
    ...
    // (Añade initiateFusion y populateFusionList si faltan aquí, copiados del paso anterior pero usando las nuevas fuentes)
