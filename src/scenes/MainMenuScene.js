...
export default class MainMenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (!this.hasLoaded) { SaveSystem.load(); if (!gameState.talents) gameState.talents = []; this.hasLoaded = true; }
    ...
    actionEquip() { 
        ...
        if (item.type === 'tower_part') { 
            ...
            this.refreshInventory(); this.switchTab('towers'); SaveSystem.save(); 
            return; 
        } 
        ...
        // TRANSACCIÓN: Borrar primero
        if (!this.removeItemFromInventory(item)) {
             this.showCentralAlert("Error al mover item", "#ff0000");
             return;
        }
    ...
    actionSell() { 
        ...
        let sellPrice = 50; 
        const rData = RARITY[realItem.rarity]; 
    ...
    executeFusion() { 
        ...
        if (result.success) { 
            // BORRADO ATÓMICO: Filtramos ambos IDs de una vez
    ...
    // En swapping y forceUnequip NO llamamos a removeItem, porque ya lo hicimos antes en actionEquip
    // Solo hacen push del item antiguo a inventario.
    ...
    forceUnequip(slot) { 
        if (gameState.equipment[slot]) { gameState.inventory.push(gameState.equipment[slot]); gameState.equipment[slot] = null; } 
    }

    // --- RESTO DE VISTAS (Héroe, Torres, Forja...) ---
    // (Copia createHeroView, refreshHero, showUnequipModal, createTowersView, refreshTowersView, 
    //  showTowerUnequipModal, createTalentsView, refreshTalents, learnTalent, createInventoryView, 
    //  createFusionModals, createInvCategoryBtn, refreshInventory, selectItem, initiateFusion, 
    //  populateFusionList, selectSecondItemForFusion, createForgeView, createForgeCatBtn, 
    //  refreshForge, selectNormalRecipe, selectTowerPart, handleCraftButton, executeCraftItem, executeCraftTower)
    
    // ... INSERTA AQUÍ EL RESTO DEL CÓDIGO DEL MENSAJE ANTERIOR QUE YA FUNCIONABA BIEN ...
    // Para simplificar, asumo que copias los métodos de UI que no cambiaron de lógica (solo actionEquip/Sell/Fusion cambiaron)
    // Aquí van: createHeroView... hasta executeCraftTower.
