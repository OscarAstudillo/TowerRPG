...
export default class MainMenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (!this.hasLoaded) {
            ...
            this.sanitizeData(); // LIMPIEZA PROFUNDA DE IDs
            this.hasLoaded = true;
        }
    ...
    // --- SANITIZACIÓN ROBUSTA (Convierte todo ID a String) ---
    sanitizeData() {
        console.log("Saneando inventario...");
        ...
        Object.values(gameState.equipment).forEach(i => { if(i) equippedIds.add(fixId(i)); });
        Object.values(gameState.towerEquipment).forEach(t => {
            if(t.slot1) equippedIds.add(fixId(t.slot1));
            if(t.slot2) equippedIds.add(fixId(t.slot2));
        });

        // Filtrar y arreglar inventario
        const cleanInv = [];
        const seenIds = new Set();
        
        gameState.inventory.forEach(item => {
            if (item) {
                fixId(item); // Asegurar ID string
                // Si el item está equipado, NO añadirlo (es fantasma)
                if (equippedIds.has(item.id)) return;
                
                // Si el ID está duplicado en inventario, regenerarlo
    ...
    // --- FUNCIÓN SEGURA PARA AÑADIR (Evita duplicados) ---
    safeAddItemToInventory(item) {
        ...
        if (!exists) {
            ...
            console.warn("Evitado duplicado en inventario:", item.name);
        }
    }
    ...
    // --- ACCIÓN DE EQUIPAR (UNIFICADA) ---
    actionEquip() { 
        if (!this.selectedItem) return; 
        
        // 1. Buscar y Borrar del Inventario (Atómico)
        ...
        if (item.type === 'tower_part') { 
            ...
            if (!gameState.towerEquipment[type].slot1) { 
                ...
                this.safeAddItemToInventory(oldItem); // Devolver seguro a mochila
                gameState.towerEquipment[type].slot1 = item; 
            } 
    ...
    executeFusion() { 
        ...
        // BORRADO POR FILTRO (Lo más seguro)
        // Guardamos las referencias antes de borrar
        const item1 = gameState.inventory.find(i => i.id === id1);
        const item2 = gameState.inventory.find(i => i.id === id2);

        if(!item1 || !item2) { this.showCentralAlert("Error: Items no encontrados", "#ff0000"); this.fusionConfirmModal.setVisible(false); return; }

        const result = RPGSystem.fuseSpecificItems(item1, item2); 
        if (result.success) { 
            // Eliminamos ambos IDs de una pasada
    ...
    // --- FUNCIÓN QUE FALTABA (Añadida explícitamente) ---
