...
export default class MainMenuScene extends Phaser.Scene {
    ...
    sanitizeData() {
        ...
        if(gameState.inventory.length !== before) console.log("Fantasmas eliminados.");
        SaveSystem.save();
    }
    ...
    actionEquip() { 
        ...
        // 1. ELIMINAR DE INVENTARIO (Siempre el primer paso)
        gameState.inventory.splice(idx, 1);

        // >> LÓGICA TORRES
        if (item.type === 'tower_part') { 
            const type = item.towerType; // 'archer', 'cannon' o 'mage'
            // Usamos lógica de SWAP manual, igual que el héroe
            if (!gameState.towerEquipment[type].slot1) {
                ...
                // Slots llenos, reemplaza Slot 1 y devuelve el viejo a mochila
        ...
        let error = ""; 
        // Validaciones... (abreviado, tu lógica original estaba bien)
        if (cls === 'arquero' && item.subType !== 'bow' && item.subType !== 'leather' && item.subType !== 'ring') error = "Clase inválida"; 
        // ... (resto de clases) ...
        if (error) { 
            // Si hay error, DEVOLVEMOS el item a la mochila (Rollback)
    ...
    refreshForge() {
        ...
        // UNIFICACIÓN: Usar loop genérico para torres también
