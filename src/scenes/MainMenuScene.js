...
export default class MainMenuScene extends Phaser.Scene {
    constructor() {
        ...
        // Estado para la Forja (Acordeón)
        this.expandedRecipeId = null; // ID de la receta desplegada
        this.expandedTowerType = null; // Tipo de torre desplegada
        
        // Estado para crafteo seleccionado
        this.craftSelection = {
            type: null, // 'item' o 'tower'
            recipe: null,
            rarity: null,
            towerType: null
        };
    ...
    // ... helpers mochila (initiateFusion, populateFusionList, etc se mantienen) ...
    initiateFusion() { if (!this.selectedItem) return; this.itemToFuse1 = this.selectedItem; this.fusionModal.setVisible(true); this.populateFusionList(); }
    populateFusionList() {
        this.fusionList.removeAll(true);
        const candidates = gameState.inventory.filter(i => i !== this.itemToFuse1 && i.type === this.itemToFuse1.type && i.rarity === this.itemToFuse1.rarity && i.enchant === this.itemToFuse1.enchant);
        if (candidates.length === 0) { this.fusionList.add(this.add.text(0, 0, "No hay items compatibles\n(Mismo Tipo, Rareza y Nivel)", { align: 'center' }).setOrigin(0.5)); return; }
        let y = 0;
        candidates.forEach(item => {
            ...
            btn.on('pointerdown', () => this.confirmFusion(item)); this.fusionList.add([btn, txt]); y += 50;
        });
    }
    confirmFusion(item2) {
        const result = RPGSystem.fuseSpecificItems(this.itemToFuse1, item2);
        if (result.success) { this.removeItemFromInventory(this.itemToFuse1); this.removeItemFromInventory(item2); gameState.inventory.push(result.item); this.fusionModal.setVisible(false); this.selectedItem = null; this.itemDetailContainer.setVisible(false); this.refreshInventory(); SaveSystem.save(); alert(`¡Fusión Exitosa! Nuevo item: ${result.item.name}`); } else { alert(result.error); }
    }
    ...
    createForgeCatBtn(x, y, label, cat) {
        ...
        btn.on('pointerdown', () => {
            ...
            this.expandedRecipeId = null; // Reset expansión al cambiar cat
    ...
    refreshForge() {
        ...
        // --- LÓGICA DE INGENIERÍA (TORRES) ---
        if (this.forgeCategory === 'tower_part') {
            const types = ['archer', 'cannon', 'mage'];
            
            types.forEach((t, index) => {
                // Botón Base (Nombre Torre)
                ...
                btn.on('pointerdown', () => {
                    // Toggle Expansión
                    this.expandedTowerType = (this.expandedTowerType === t) ? null : t;
                    this.refreshForge();
                });
                this.recipesContainer.add([btn, txt]);
                startY += 55;

                // Si está expandido, mostrar rarezas debajo
                if (this.expandedTowerType === t) {
                    ['common', 'uncommon', 'rare', 'epic', 'legendary'].forEach(rarity => {
                        ...
                        const rTxt = this.add.text(startX + (col * 250) + 20, startY, rData.name, {fontSize:'12px', color: '#' + rData.color.toString(16)}).setOrigin(0.5);
                        
                        rBtn.on('pointerdown', () => this.selectTowerPart(t, rarity));
                        this.recipesContainer.add([rBtn, rTxt]);
                        startY += 40;
                    });
                    startY += 10; // Espacio extra tras lista
                }
            });
            return;
        }

        // --- LÓGICA DE EQUIPAMIENTO NORMAL (AGRUPADO) ---
        // 1. Obtener recetas únicas por ID (ignorando duplicados si los hubiera)
        // RECIPES es una lista plana, así que filtramos por categoría
        ...
        categoryRecipes.forEach(recipe => {
            ...
            btn.on('pointerdown', () => {
                this.expandedRecipeId = (this.expandedRecipeId === recipe.id) ? null : recipe.id;
                this.refreshForge();
            });
            this.recipesContainer.add([btn, txt]);
            startY += 55;
            ...
            // Control de columnas si la lista es muy larga (opcional)
            if (startY > h * 0.8) {
                col++;
                startY = h * 0.35;
            }
    ...
    // --- SELECCIÓN PREVIA (Previsualizar) ---
    ...
    selectTowerPart(towerType, rarityKey) {
        this.recipeDetailContainer.setVisible(true);
        this.craftSelection = { type: 'tower', towerType: towerType, rarity: rarityKey };

        const rarity = RARITY[rarityKey];
        const hexColor = '#' + rarity.color.toString(16).padStart(6, '0');
        
        this.recipeTitle.setText(`Mejora ${towerType.toUpperCase()}`);
        this.recipeTitle.setColor(hexColor);

        // Información ficticia para preview
    ...
    handleCraftButton() {
        ...
        if (sel.type === 'item') {
            ...
            this.executeCraftTower(sel.towerType, sel.rarity);
        }
    }
