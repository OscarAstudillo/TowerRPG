...
export default class GameScene extends Phaser.Scene {
    ...
    init(data) {
        ...
        // --- 1. CARGAR TEMA DEL NIVEL ---
        // Si no tiene tema (ej. pruebas), usamos colores por defecto (Gris)
        this.theme = this.currentLevelData.theme || {
            background: 0x333333,
            path: 0x555555,
            accent: 0x00ffff // Cian por defecto
        };
    ...
    create() {
        ...
        // El fondo fuera del mapa será un tono muy oscuro del color del bioma
        ...
        // --- 2. DIBUJAR MAPA CON COLORES DEL TEMA ---
        ...
        // Borde del camino (Detalle estético)
    ...
    createUI() {
        const uiDepth = 1000;
        const accentColor = this.theme.accent; // Color especial del nivel

        // TOP BAR
        this.add.rectangle(640, 60, 1280, 120, 0x111111).setScrollFactor(0).setDepth(uiDepth);
        // Línea de color temático
        this.add.rectangle(640, 120, 1280, 4, accentColor).setScrollFactor(0).setDepth(uiDepth); 
        
        this.livesText = this.add.text(30, 25, '', { fontSize: '20px', fontStyle: 'bold', color: '#fff' }).setScrollFactor(0).setDepth(uiDepth + 1);
        // Texto de oleada con color temático
        this.waveInfoText = this.add.text(1250, 40, 'OLEADA: 1', { fontSize: '28px', fontStyle: 'bold', color: '#ffffff' }).setOrigin(1, 0.5).setScrollFactor(0).setDepth(uiDepth + 1);
        ...
        // Línea de color temático abajo
        this.add.rectangle(640, 840, 1280, 4, accentColor).setScrollFactor(0).setDepth(uiDepth);
        ...
        // Texto de selección con color temático
        this.buildText = this.add.text(300, 890, '', { fontSize: '20px', color: accentColor }).setScrollFactor(0).setDepth(uiDepth + 1);
        ...
        // Barra de skill con color temático
        this.skillBar = this.add.rectangle(-100, 0, 0, 80, accentColor).setOrigin(0, 0.5); 
    ...
    startNextWave() {
        ...
        else { this.waveInfoText.setText(`OLEADA: ${this.currentWave}/${this.totalWaves}`); this.waveInfoText.setColor(this.theme.accent); }
    ...
    updateSkillUI() { 
        ...
        if (cd > 0) { 
            ...
            this.skillBar.setFillStyle(this.theme.accent); // Color temático
