...
export default class GameScene extends Phaser.Scene {
    ...
    init(data) {
        this.currentLevelData = data.levelData || { id: 1, name: "Nivel Prueba", startCoins: 500, difficulty: 1, path: [], towerSlots: [] };
    }
    ...
    collectLoot(lootItem) {
        // 1. Guardar en Global (Persistencia)
        gameState.materials[lootItem.typeKey][lootItem.rarityKey]++;
        
        // 2. Guardar en Local (Para el resumen de victoria)
    ...
    victory() {
        ...
        this.physics.pause(); // Detener el juego
        ...
        const title = this.add.text(640, 180, "¡VICTORIA!", { fontSize: '40px', color: '#00ff00', fontStyle: 'bold' }).setOrigin(0.5);
        const subTitle = this.add.text(640, 230, "Recompensas Obtenidas:", { fontSize: '20px', color: '#fff' }).setOrigin(0.5);

        // Texto del Oro
        const goldTxt = this.add.text(640, 270, `💰 Oro: +${goldReward}`, { fontSize: '24px', color: '#ffd700' }).setOrigin(0.5);
        ...
        if (lootList === "") lootList = "(No recogiste materiales)";

        const matsTxt = this.add.text(640, 350, lootList, { fontSize: '16px', color: '#aaa', align: 'center', lineHeight: 24 }).setOrigin(0.5);

        // Botón Continuar
        const btn = this.add.rectangle(640, 480, 200, 50, 0x006400).setInteractive({ useHandCursor: true });
        const btnTxt = this.add.text(640, 480, "CONTINUAR", { fontSize: '20px', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    // --- RESTO DE FUNCIONES (Oleadas, UI, etc) ---
    // (Estas se mantienen igual que antes, las incluyo para que el archivo esté completo y sin errores)
    ...
    createUpgradeUI() {
        ...
        this.upgradeBtn.on('pointerdown', () => { if (this.selectedTowerToUpgrade) this.tryUpgradeTower(); });
    }
