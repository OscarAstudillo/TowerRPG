...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.towers = this.physics.add.group({ classType: Tower, runChildUpdate: false }); // Update manual para torres
    ...
    startNextWave() {
        ...
        if (this.currentWave === 5) {
            count = 1; interval = 1000; hpMult = 2.5; speedMult = 0.4; isBoss = true; 
            this.waveInfoText.setText("OLEADA: BOSS!"); this.waveInfoText.setColor('#ff0000');
        } else {
            this.waveInfoText.setText(`OLEADA: ${this.currentWave}/${this.totalWaves}`); this.waveInfoText.setColor('#00ffff');
        }
    ...
    createUI() {
        ...
        // --- CORRECCIÓN 2: BOTÓN OLEADA ROBUSTO ---
        this.waveTimerContainer = this.add.container(640, 60).setScrollFactor(0).setDepth(uiDepth + 2);
        
        const timerBg = this.add.rectangle(0, 0, 320, 60, 0x006400)
            .setInteractive({ useHandCursor: true })
            .setStrokeStyle(2, 0xffffff);
            
        this.waveTimerBtnText = this.add.text(0, 0, "INICIAR", { fontSize: '18px', fontStyle: 'bold', align: 'center' })
            .setOrigin(0.5)
            .setInteractive({ useHandCursor: true }); // TEXTO TAMBIÉN INTERACTIVO

        this.waveTimerContainer.add([timerBg, this.waveTimerBtnText]);
        this.waveTimerContainer.setVisible(false);

        // Ambos disparan la acción
        timerBg.on('pointerdown', () => this.startNextWaveAction());
        this.waveTimerBtnText.on('pointerdown', () => this.startNextWaveAction());
    ...
    // ... (Mantén createBuildSlots, tryBuildTower, createUpgradeUI, openUpgradeMenu, closeUpgradeMenu, updateUpgradeMenuText, tryUpgradeTower, sellTower, checkWaveStatus, spawnLoot, collectLoot, onEnemyLeaks, gameOver, victory, triggerPlayerSkill, createSpawnIndicator igual que antes) ...
    // Aquí repito las esenciales para el copy-paste seguro:
    ...
    // MENÚ UPGRADE/VENTA (Asegúrate de tener esto, es la lógica de venta)
    createUpgradeUI() {
    ...
    tryUpgradeTower() {
        ...
        if (t && this.coins >= t.upgradeCost) {
            this.coins -= t.upgradeCost; t.upgrade();
            this.updateUpgradeMenuText(); this.updateUI();
        }
    }
    sellTower() {
        const t = this.selectedTowerToUpgrade;
        if (t) {
            this.coins += t.totalInvestment; this.updateUI();
            if (t.buildSite) t.buildSite.free();
            t.destroy(); this.closeUpgradeMenu();
