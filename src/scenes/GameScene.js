...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.input.on('gameobjectdown', (pointer, gameObject) => {
            if (this.isPaused) return; // No clickear si está en pausa
    ...
    update(time, delta) {
        if (this.isPaused) return; // Detener lógica si está en pausa
    ...
    // --- SISTEMA DE PAUSA ---
    createPauseMenu() {
        // Contenedor que ignora la cámara del juego (se pega a la pantalla)
        this.pauseContainer = this.add.container(640, 480).setDepth(3000).setVisible(false).setScrollFactor(0);
        
        // Fondo oscuro
        const bg = this.add.rectangle(0, 0, 1280, 960, 0x000000, 0.7).setInteractive(); // Bloquea clics
    ...
    togglePause() {
        ...
        if (this.isPaused) {
            this.physics.pause(); // Detener físicas (movimiento enemigos)
            this.pauseContainer.setVisible(true);
            // Pausar tweens y timers
    ...
    // Copia aquí todas las funciones que no cambiaron para asegurar que el archivo funcione:
    // createParticles, createExplosion, showFloatingText, showLevelUpEffect, createUI, updateUI, updateSkillUI, startNextWave...
    // IMPORTANTE: Asegúrate de incluir startNextWave con la lógica de oleadas variadas que hicimos antes.
    
    // Repito las esenciales para el copy-paste seguro:
    createParticles() {
